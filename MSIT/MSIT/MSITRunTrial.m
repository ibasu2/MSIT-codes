...
try
    while (sessionVars.trialNum < MSITOpts.trialParams.numTrials) && ~sessionVars.quitTask
        ...
        trialData = MSITInitiateTrial(MSITOpts,MSITTaskSpecs,sessionVars,lslInlet);
        ...
        %draw & show stimulus
        % createAndFlipPhotoDiodeRect(visEnviro); %flip photodiode
        ...
        % Screen(visEnviro.screen.window,'FillRect',visEnviro.rig.colorDepth/2);
        % createAndFlipPhotoDiodeRect(visEnviro); %flip photodiode
        ...
        display(['Lag between RT photodiode: ',num2str(GetSecs-trialData.responseTime)])
...
function trialData = MSITInitiateTrial(MSITOpts,MSITTaskSpecs,sessionVars,lslInlet)
