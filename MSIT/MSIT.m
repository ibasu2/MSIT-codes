function MSIT(subjID,showInstructions,usingLSL,useTaskTriggeredStim,UpdateOpts)
% MSIT.m
% The new Matlab version of the MSIT (Multisource Interference Task) task
% without efficacy!
% Written by Seth Konig 11/3/2022 based on MSIT with efficacy part of the
% code stripped out!
%
% Inputs:
...
%   5) (AS) UpdateOpts: Parameters to change from default MSIT_params
%   function
...
try %for task setup
    ...
    %If UpdateOpts specified, update MSITOpts fields
    if nargin > 4
       updatedFields = fields(UpdateOpts);
       for ii=1:numel(updatedFields)
           MSITOpts.(updatedFields{ii}) = UpdateOpts.(updatedFields{ii});
       end
    end
